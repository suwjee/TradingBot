...
`prepare_market_context()` reads the complete physical RAW JSON file, strips an optional UTF-8 BOM, parses with `orjson`, and gives **all rows** to `build_candle_buckets()`. Requested `from_time` and `to_time` are a presentation window; they do not initialize a new empty trading state or truncate calculation. Later RAW events can resolve a Reaction whose First is inside the requested display window. A displayed object's confirmation or stop may consequently occur outside that window.
...
Main candle indexes refer to the full file. Requested bounds are floored to bucket opens; binary search selects through the bucket containing `to_time`, inclusive. `actualFrom` and `actualTo` report selected main-candle opens. Display strings use `Asia/Tehran`; public JSON times are epoch seconds. Time, source index, source candle time and exact event time are separate provenance fields and must not be substituted for one another.
